...
export class ComponentsContainer {
    ...
    registerComponent(serviceContainer: Container,  cls: any) {
    
        const inst = cls.getInstance
        ? cls.getInstance({ services: serviceContainer })
        : new cls({ services: serviceContainer});

        const my_config = config.services?.[inst.uid] || {};
    ...
    async init() {
        for (const k in this.instances_) {
            if (this.instances_[k] instanceof BaseComponent) {
