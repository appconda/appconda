...
export class KV {
    ...
    expire(key, seconds, options = {}) {
        ...
        const { NX = false, XX = false, GT = false, LT = false }: any = options;
    ...
    sscan(key, cursor, options = {}) {
        const { match = '*', count = 10 }: any = options;
        const set = this.store.get(key);
        if (set === undefined) {
            return [0, []];
        }
    ...
    */
    lpos(key, element, options = {}) {
        const { rank = 0, start = 0, stop = -1 }: any = options;
        const list = this.store.get(key);
    ...
    blmove(source, destination, srcDirection, destDirection, timeout) {
        ...
        const element: any = this[popFn]([source], timeout);
        if (element === undefined) {
            return null;
        }
    ...
    blmpop(count, timeout, ...keys) {
        ...
        for (let i = 0; i < count; i++) {
            const value = (this as any)[popFn](popArgs);
            if (value === undefined) {
                break;
            }
            results.push(value);
     ...
     */
    zdiffstore(destination, ...keys: any) {
        const diff = (this as any).ZDIFF(...keys);
    ...
    bzmpop(count, ...keys) {
        ...
        for (const key of keys) {
            ...
            if (sortedSet && sortedSet.size > 0) {
                const poppedMembers = (Array as any).from(sortedSet.entries())
                    .sort((a, b) => a[1] - b[1])
                    .slice(0, count)
                    .map(([member, score]) => {
     ...
     */
    zintercard(...keys) {
        const intersection = (this as any).ZINTER(...keys);
    ...
    zlexcount(key, min, max) {
        ...
        const sortedMembers: any = Array.from(sortedSet.keys()).sort();
        let count = 0;
    ...
    zrange(key, start, stop) {
        const sortedSet = this.store.get(key) || new XMap();
        const sortedMembers = (Array as any).from(sortedSet.entries()).sort((a, b) => a[1] - b[1]);
     ...
     */
    zrangebylex(key, min, max, options: any = {}) {
        const sortedSet = this.store.get(key) || new XMap();
        const sortedMembers: any = Array.from(sortedSet.keys()).sort();
     ...
     */
    zrangebyscore(key, min, max, options: any = {}) {
        const sortedSet = this.store.get(key) || new XMap();
        const sortedMembers = (Array as any).from(sortedSet.entries()).sort((a, b) => a[1] - b[1]);
    ...
    zremrangebylex(key, min, max) {
        ...
        const sortedMembers = (Array as any).from(sortedSet.keys()).sort();
        let removedCount = 0;
    ...
    zrevrange(key, start, stop) {
        const sortedSet = this.store.get(key) || new XMap();
        const sortedMembers = (Array as any).from(sortedSet.entries()).sort((a, b) => b[1] - a[1]);
     ...
     */
    zrevrangebylex(key, max, min, options: any = {}) {
        const sortedSet = this.store.get(key) || new XMap();
        const sortedMembers = (Array as any).from(sortedSet.keys()).sort().reverse();
     ...
     * @returns {Array} - The specified range of elements in reverse order.
     */
    zrevrangebyscore(key, max, min, options: any = {}) {
        const sortedSet = this.store.get(key) || new XMap();
        const sortedMembers = (Array as any).from(sortedSet.entries()).sort((a, b) => b[1] - a[1]);
    ...
    zunionstore(destination, keys) {
        const unionResult: any = this.zunion(keys);
    ...
    georadius_ro(latitude, longitude, radius, key) {
        return (this as any).georadius(latitude, longitude, radius, key, true);
    }
    ...
    georadiusbymember_ro(key, member, radius) {
        const coord = this.geopos(key, member);
        if (!coord) return undefined;
        return (this as any).georadius(coord[0], coord[1], radius, key, true);
    }
    ...
    hscan(key, cursor, match = '*', count = 10) {
        const hashMap = this.store.get(key) || new XMap();
        const filteredFields = (Array as any).from(hashMap.keys()).filter(field => field.includes(match));
