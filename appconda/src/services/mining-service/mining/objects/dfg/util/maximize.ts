...
export class FilteredDfgMaximization {
	static apply(freqDfg) {
		...
		let constraintMatrix: any = [];
		let constraintsVector: any = [];
		...
		for (let pind in pathsKeys) {
			let constraintRow: any = zeroRow.slice();
		...
		for (let actInd in activitiesKeys) {
			...
			for (let act2 in ingoing[act]) {
				let path = [act2, act];
				let pind: any = pathsKeys.indexOf(path.toString());
				constraintRow[parseInt(pind)] = 1;
			}
