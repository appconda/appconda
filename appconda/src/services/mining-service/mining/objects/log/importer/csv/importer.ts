import { EventLog, Event, Attribute, Trace } from "../../log";

export class CsvImporter {
	static DEFAULT_SEPARATOR: any;
	static DEFAULT_QUOTECHAR: any;
	static DEFAULT_CASE_ID: any;
	static DEFAULT_ACTIVITY: any;
	static DEFAULT_TIMESTAMP: any;
	static DEFAULT_CASE_ID_AS_TRACE_ATTRIBUTE: any;
	static DEFAULT_CASE_PREFIX: string;
	static apply(str, sep=CsvImporter.DEFAULT_SEPARATOR, quotechar=CsvImporter.DEFAULT_QUOTECHAR, caseId=CsvImporter.DEFAULT_CASE_ID, activity=CsvImporter.DEFAULT_ACTIVITY, timestamp=CsvImporter.DEFAULT_TIMESTAMP) {
		let csvArray: any = CsvImporter.parseCSV(str, sep=sep, quotechar=quotechar);
		let traces = {};
		let i = 1;
		let j = 0;
