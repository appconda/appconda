...
export class IntervalTreeBuilder {
...
export class IntervalTreeAlgorithms {
	static resourceWorkload(tree, pointOfTime: any=null, resourceKey="org:resource") {
		if (pointOfTime == null) {
	...
	static targetActivityWorkload(tree, pointOfTime: any=null, activityKey="concept:name") {
		if (pointOfTime == null) {
			pointOfTime = (tree.mintime + tree.maxtime) / 2.0;
		}
		let contained = tree.queryPoint(pointOfTime);
	...
	static sourceActivityWorkload(tree, pointOfTime: any=null, activityKey="concept:name") {
		if (pointOfTime == null) {
			pointOfTime = (tree.mintime + tree.maxtime) / 2.0;
		}
		let contained = tree.queryPoint(pointOfTime);
	...
	static queryInterval(tree, st, et) {
		...
		let afterIntersectingObjects: any = [];
		let beforeIntersectingObjects: any = [];
		...
		let intersectionAfterBefore: any = [];
