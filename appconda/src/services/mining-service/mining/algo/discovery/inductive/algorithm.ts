...
export class InductiveMiner {
	...
	static apply(eventLog, activityKey: any="concept:name", threshold=0.0, freqDfg=null, removeNoise=false, variantsFiltering=false) {
		if (variantsFiltering) {
			if (threshold > 0.0) {
				...
				for (let k in variants) {
					let v = variants[k];
					variants2.push([k, v] as any);
					totalCount += 1;
				}
				variants2.sort((a, b) => b[1] - a[1]);
				let includedVariants: any = [];
				let i = 0;
				let partialSum = 0;
	...
	static inductiveMiner(log, treeParent, activityKey, removeNoise, threshold, freqDfg: any=null, skippable=false) {
		if (log != null) {
			if (threshold == 0) {
		...
		if (!(removeNoise)) {
			...
			let detectedFallthrough: any = null;
			if (log != null) {
	...
	static detectCut(log, freqDfg, treeParent, activityKey, threshold) {
		...
		let vect: any = null;
		let subdfgs: any = null;
		let skippable: any = null;
		if (seqCut != null) {
	...
	static detectFallthroughsDfg(freqDfg, treeParent, threshold) {
		let activityConcurrentCut: any = InductiveMinerActivityConcurrentFallthroughDFG.detect(freqDfg, threshold);
...
export class InductiveMinerSequenceCutDetector {
    // Basic Steps:
	...
	static projectDfg(dfg, groups) {
		let startActivities: any = [];
		let endActivities: any = [];
		let activities: any = [];
		let dfgs: any = [];
		let skippable: any = [];
		for (let g of groups) {
...
export class InductiveMinerLoopCutDetector {
	...
	static detect(freqDfg0, activityKey) {
		...
		let connComp: any = InductiveMinerGeneralUtilities.getConnectedComponents(freqDfg);
	...
	static projectDfg(frequencyDfg, groups: any) {
		let dfgs: any = [];
...
export class InductiveMinerActivityConcurrentFallthroughDFG {
	...
	static projectDfg(frequencyDfg, act) {
		return InductiveMinerActivityConcurrentFallthroughDFG.removeActFromDFG(frequencyDfg, act);
	}
}

export class InductiveMinerStrictTauLoopFallthrough {
	static detect(log, freqDfg, activityKey) {
