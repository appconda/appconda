import { LogSkeleton } from "../../../objects/skeleton/log_skeleton";
import { GeneralLogStatistics } from "../../../statistics/log/general";

export class LogSkeletonDiscovery {
	static apply(eventLog, activityKey="concept:name") {
		...
		let neverTogether: any = {};
		let equivalence: any = {};
		let equivalenceTotCases: any = {};
		let alwaysAfter: any = {};
		let alwaysBefore: any = {};
		let directlyFollows: any = {};
		let actCounter = {};
		for (let trace of eventLog.traces) {
			...
			while (i < trace.events.length - 1) {
				...
				while (j < trace.events.length) {
					...
					let cou1: any = [acti, actj];
					let cou2: any = [actj, acti];
