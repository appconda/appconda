import { BusinessHours } from "../../../utils/generic/business_hours";
...
export class TemporalProfileDiscovery {
	static getAvg(vect) {
		if (vect.length > 0) {
			let sum = 0.0;
			for (let el of vect) {
		...
		for (let trace of eventLog.traces) {
			...
			while (i < trace.events.length - 1) {
				...
				while (j < trace.events.length) {
					...
					if (!([act_i, act_j] as any in actCouplesTimes)) {
						actCouplesTimes[[act_i, act_j] as any] = [];
					}
					actCouplesTimes[[act_i, act_j] as any].push(diff);
					j++;
				}
				i++;
			}
