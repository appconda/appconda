...
import { ExponentialRandomVariable } from "../../../../utils/random_variables/expon";

export class PerformanceDfgSimulation {
	...
	static apply(performanceDfg: any, numDesideredTraces=1000, activityKey="concept:name", timestampKey="time:timestamp", caseArrivalRate=1) {
		...
		while (i < numDesideredTraces) {
			...
			while (currActivity != "■") {
				...
				if (currActivity != "▶" && nextActivity != "■") {
					let path: any = [currActivity, nextActivity];
