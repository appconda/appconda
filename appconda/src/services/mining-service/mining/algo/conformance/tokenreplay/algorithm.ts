export class TokenBasedReplayResult {
	acceptingPetriNet: any;
	result: any;
	...
	transExecutions: any;
	arcExecutions: any;
	totalConsumedPerPlace: any;
	totalProducedPerPlace: any;
	totalMissingPerPlace: any;
	totalRemainingPerPlace: any;
	transExecutionPerformance: any;
	totalTraces: any;
	fitTraces: number;
	logFitness: number;
...
export class TokenBasedReplay {
	static apply(eventLog, acceptingPetriNet, activityKey = "concept:name", reachFm = true, oneReplayPerTrace = true, timestampKey = "time:timestamp") {
		...
		let dictioResultsVariant: any = {};
		let ret: any = [];
		for (let trace of eventLog.traces) {
			let arrActivities: any = TokenBasedReplay.getArrActivities(trace, activityKey);
	...
	static applyListListAct(listListActivities, acceptingPetriNet, reachFm = true, retMarking = false) {
		...
		for (let activities of listListActivities) {
			...
			let tbrResult: any = TokenBasedReplay.performTbr(activities.split(","), transitionsMap, acceptingPetriNet, invisibleChain, reachFm, arrTimestamp);
			if (retMarking) {
	...
	static performTbr(activities, transitionsMap, acceptingPetriNet, invisibleChain, reachFm, arrTimestamp) {
		...
		let visitedTransitions: any = [];
		let visitedTimes: any = [];
		let visitedMarkings: any = [];
		let visitedMarkingsTimes: any = [];
		let missingActivitiesInModel: any = [];
		let mainidx = 0;
		...
		while (mainidx < activities.length) {
			...
			if (act in transitionsMap) {
				...
				let newVisitedTransitions: any = [];
				let newVisitedTimes: any = [];
				let newVisitedMarkings: any = [];
				let newVisitedMarkingsTimes: any = [];
