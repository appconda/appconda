...
export class PetriNetAlignments {
	static apply(log, acceptingPetriNet, activityKey="concept:name", maxExecutionTime=Number.MAX_VALUE, syncCosts: any=null, modelMoveCosts: any=null, logMoveCosts: any=null) {
		...
		if (modelMoveCosts == null) {
			...
			for (let transId in acceptingPetriNet.net.transitions) {
				...
				if (trans.label == null) {
					...
					let visibleEnabledTransitions: any = [];
		...
		try {
			minPathInModelCost = Math.floor((PetriNetAlignments as any).applyTrace([], acceptingPetriNet.net, acceptingPetriNet.im, acceptingPetriNet.fm, syncCosts, modelMoveCosts, logMoveCosts, comparator, maxExecutionTime)["cost"] / 10000);
		}
		catch (err) {
		}
