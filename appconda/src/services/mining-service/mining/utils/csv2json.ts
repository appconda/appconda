...
function zip() {
    ...
    var longest = args.reduce(function (a: any, b: any) {
...
export class CvsToJson {
    public static Convert(csv, options) {
        options || (options = { });
        ...
        var a: any = [];
        try {
            var a = CvsToJson.parse(csv, pegjsSeparatorNames[separator]);
        } catch (error) {
            var start = csv.lastIndexOf('\n', (error as any).offset),
                end = csv.indexOf('\n', (error as any).offset),
                line = csv.substring(start >= -1 ? start : 0, end > -1 ? end : csv.length);
            throw (error as any).message + ' On line ' + (error as any).line + ' and column ' + (error as any).column + '.\n' + line;
        }
     ...
     */
    public static parse(input, startRule) {
        var parseFunctions = {
            "comma": parse_comma,
        ...
        var rightmostFailuresExpected: any = [];
...
export class SyntaxError extends Error {
    ...
    line: any;
    column: any;
    public constructor(expected, found, offset, line, column) {
        super();
