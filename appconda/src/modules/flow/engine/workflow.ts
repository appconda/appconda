...
export interface ExecutionInterface {
 ...
 async function fillVariables(schema, variables) {
  const inputs = schema;
  for (let key in inputs) {
      if (typeof inputs[key] === 'string') {
        try {
          ...
          const result: any = await _html(variables);
          ...
          await fillVariables(inputs[key], variables);
      }
  }
}
/**
 ...
 */
async function run(target: ComponentsContainer, method: string, options: MethodOptions, is_first_iteration?: boolean) {
  options.activity.token = options.token;
  options.activity.context = options.context;

  const componentName = (options as any).activity.$.component;
  const inputs = (options as any).activity.$.inputs;

  await fillVariables(inputs, options.value);
  ...
  try {
    ...
    if (!componentName && !node?.options?.pause) {
      ...
      const comObject = target.get(componentName);

      value = await comObject.build(inputs); //target[method](options);
      options.activity.takeOutgoing();
 ...
 */
    ...
    this.process = this.process ?? getBPMNProcess(this.definition, { name: 'first' } as any);
    ...
    if (context.tokens.length == 0) {
      const state = State.build(activity.id, { name: activity.name, component: (activity as any).$.component, value } as any);
    ...
    do {
      const result = await run((options as any).componentsContainer, runOptions.method, runOptions.options, is_first_iteration);
      ...
      if (context.status === Status.Running) {
        ...
        if (!(next as any).component && result.value) val = { [token.id]: result.value };
        else if ((next as any).component && val[token.id]) {
        ...
        if (activity['$'] && (activity as any)['$'].inputs) {
          let bufferObj = Buffer.from((activity as any)['$'].inputs, "base64");

          // Encode the Buffer as a utf8 string 
          let decodedString = bufferObj.toString("utf8");
          (activity as any)['$'].inputs = JSON.parse(decodedString);
        }
